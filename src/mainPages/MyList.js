...
class MyList extends Component {
    ...
    render() {
        ...
        const { isLoading, totalInViewport, totalInList } = this.state;
        if (!isLoading) {
            let totalRows = 0;
            if (totalInList * totalInViewport !== 0) {
                totalRows = Math.ceil(totalInList / totalInViewport);
            }

            console.log(" list - totalInViewport : ", totalInViewport);
            console.log(" list - totalInList : ", totalInList);
...
const mapStateToProps = ({ video }) => {
    const { cartList, categoryList, isJawOpen } = video;
    return { cartList, categoryList, isJawOpen };
}
